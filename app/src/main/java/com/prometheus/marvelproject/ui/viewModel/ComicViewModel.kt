...
import com.prometheus.marvelproject.model.ComicFiltered
...
class ComicViewModel : BaseViewModel() {
    ...
    val onComicsListLoaded : MutableLiveData<ComicFiltered> by lazy {
        MutableLiveData<ComicFiltered>()
    }
    ...
    fun loadComics(){
        ...
        disposables.add(
            MarvelProjectApp.appComponent.MarvelService()
                .getComics(ts,PUBLIC_KEY!!,hash)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({
                    val result = it.results
                    val resultData = obtainItems(result!!)
                    onComicsListLoaded.value = ComicFiltered(resultData)
                },{
                    it.printStackTrace()
                    onError.value = "Error"
                })
    ...
    @RequiresApi(Build.VERSION_CODES.KITKAT)
    private fun obtainItems(result: Any): MutableList<Comic>? {
        var items = mutableListOf<Comic>()
        val gson = Gson()
        ...
        list.forEach { dt ->
            if (dt.isJsonObject) {
                ...
                val singleItem = Comic(id,name,description,series.toString(),comics.toString(),events.toString(),creators.toString(),characters.toString(),stories.toString())
