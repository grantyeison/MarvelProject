...
import com.prometheus.marvelproject.model.Story
import com.prometheus.marvelproject.model.StoryFiltered
...
class StoryViewModel : BaseViewModel() {
    ...
    val onStoriesListLoaded : MutableLiveData<StoryFiltered> by lazy {
        MutableLiveData<StoryFiltered>()
    }
    ...
    fun loadStories(){
        ...
        disposables.add(
            MarvelProjectApp.appComponent.MarvelService()
                .getStories(ts,PUBLIC_KEY!!,hash)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({
                    val result = it.results
                    val resultData = obtainItems(result!!)
                    onStoriesListLoaded.value = StoryFiltered(resultData)
                },{
                    it.printStackTrace()
                    onError.value = "Error"
                })
    ...
    @RequiresApi(Build.VERSION_CODES.KITKAT)
    private fun obtainItems(result: Any): MutableList<Story>? {
        var items = mutableListOf<Story>()
        val gson = Gson()
        ...
        list.forEach { dt ->
            if (dt.isJsonObject) {
                ...
                val singleItem = Story(id,name,description,series.toString(),comics.toString(),events.toString(),creators.toString(),characters.toString(),stories.toString())
